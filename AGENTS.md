...
URLは1件のこともあれば、複数行まとめて渡されることもある。複数の場合は各URLに対して同じ手順を繰り返す。
...
**既に存在する場合はその曲をスキップし、残りの登録処理は続行する。**
処理を中断したり、スキップの可否をユーザーに確認したりしない。

- 一度に複数のURLを渡された場合も、重複した分だけを飛ばして残りを登録する
- 渡されたリストの中で同じURLが重複している場合も、最初の1件だけ登録する
- **スキップした曲は最後にまとめて報告する**(黙って飛ばさない)。既存の登録内容(曲名・status)も添える

同じ曲の別動画(別 video ID)は重複ではないので、通常どおり登録する。
...
最後に `git diff` の内容と、**重複でスキップした曲があればその一覧**をユーザーに示す。
